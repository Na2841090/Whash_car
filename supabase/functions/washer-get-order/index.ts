// supabase/functions/washer-get-order/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    // 🔥 5. VERIFY WASHER EXISTS AND IS ACTIVE
    ...
    // 🔥 6. FETCH ORDER (MUST BELONG TO THIS WASHER)
    ...
    if (error || !order) {
      return new Response(
        JSON.stringify({
          ...
          details: error?.message || 'Order does not exist or is not assigned to you',
        }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    console.error('❌ Washer get order error:', error);
    return new Response(
      JSON.stringify({
        success: false,
